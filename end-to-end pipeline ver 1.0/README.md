...
For LLM calls, set `CLAUDE_CODE_OAUTH_TOKEN` (generated by `claude setup-token` against a Claude Pro/Max/Enterprise subscription). The bot sends it as `Authorization: Bearer <token>` against `https://api.anthropic.com/v1/messages` with `anthropic-beta: oauth-2025-04-20`, and prepends the required Claude Code identity to the system prompt. `OPENAI_OAUTH_TOKEN` is still accepted as a fallback when no Claude token is set.
